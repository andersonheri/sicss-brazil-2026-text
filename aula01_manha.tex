...
\begin{frame}{Onde a manhã começa e onde ela termina}
\centering
\begin{tikzpicture}[node distance=0.45cm and 0.32cm]
  \node[destaque, minimum width=1.75cm] (a) {1. Corpus\\e limpeza};
  ...
  \node[bloco, minimum width=1.8cm, right=of d] (e) {5. Auditoria\\$\kappa$, $\alpha$, AC1};
...
\small \textbf{Hoje de manhã:} etapa 1 na prática (Demos 1 e 2), e o
vocabulário conceitual de todas as outras, principalmente a etapa 5
(métricas de confiabilidade, o paradoxo do $\kappa$).
\column{0.49\textwidth}
\small \textbf{Hoje à tarde:} as etapas 2 a 6 na prática, aplicadas ao
projeto de cada um.
\end{columns}
\vspace{0.2cm}
\footnotesize \textit{Por que mostrar isso agora:} para você situar cada
slide de hoje dentro do pipeline inteiro, e não confundir ``ainda não
chegamos lá'' com ``isso não existe''.
\end{frame}
...
\begin{frame}[fragile]{Demo 3. Descrever: nuvem de palavras}
\begin{columns}[T]
\column{0.44\textwidth}
\begin{lstlisting}
...
\end{lstlisting}
\vspace{0.15cm}
\footnotesize A versão de impacto visual da Figura 1 (frequência): mesmo
dado, tamanho da fonte no lugar do comprimento da barra.
\vspace{0.15cm}
\begin{alertblock}{Cuidado}
\small Tamanho de fonte não é uma escala perceptualmente linear: o olho
superestima palavras grandes. Para leitura analítica precisa, a barra
continua sendo mais honesta que a nuvem.
\end{alertblock}
\column{0.55\textwidth}
\centering
\includegraphics[width=0.85\linewidth]{figuras/fig_wordcloud.png}\\
\scriptsize \textit{Saída ilustrativa, gerada para fins didáticos.}
\end{columns}
\end{frame}

\begin{frame}[fragile]{Demo 3. Descrever: nuvem comparativa entre grupos}
\begin{columns}[T]
\column{0.44\textwidth}
\begin{lstlisting}
...
\end{lstlisting}
\vspace{0.15cm}
\footnotesize A versão visual da Figura 2 (\emph{keyness}): usa TF-IDF,
tratando cada grupo como um "documento", para destacar o vocabulário
distintivo de cada lado, lado a lado.
\vspace{0.1cm}
\footnotesize Ganha em leitura rápida, perde em rigor: aqui não há
valor-p nem intervalo, só o ranking de TF-IDF.
\column{0.55\textwidth}
\centering
\includegraphics[width=\linewidth]{figuras/fig_wordcloud_comparativo.png}\\
\scriptsize \textit{Saída ilustrativa, gerada para fins didáticos.}
\end{columns}
\end{frame}
...
\begin{frame}[fragile]{Como escolher k: existe uma métrica}
\begin{columns}[T]
\column{0.44\textwidth}
\begin{lstlisting}
...
\end{lstlisting}
\footnotesize \texttt{ac\_lda\_tune()} testa vários $k$ e calcula a
\textbf{perplexidade}: quanto menor, melhor o modelo prevê palavras que
não viu.
\column{0.55\textwidth}
\centering
\includegraphics[width=\linewidth]{figuras/fig_lda_tune.png}\\
\scriptsize \textit{Saída ilustrativa, gerada para fins didáticos.}
\end{columns}
\footnotesize \textbf{Leia o ``cotovelo'', não o mínimo.} Piso, não
teto: a palavra final ainda depende de ler os tópicos.
\end{frame}

\begin{frame}{LDA ou STM? Depende da sua pergunta}
\begin{columns}[T]
\column{0.48\textwidth}
\small \textbf{LDA (o que rodamos)}
\begin{itemize}\footnotesize
  \item Tópicos ``soltos'', sem metadado do corpus.
  \item Responde: \emph{quais tópicos existem?}
  \item Comparar com partido/período é manual, depois.
\end{itemize}
\column{0.48\textwidth}
\small \textbf{STM (pacote \texttt{stm})}
\begin{itemize}\footnotesize
  \item Covariável \textbf{dentro} do modelo: na prevalência (quanto se
        fala) e no conteúdo (como se fala).
  \item Responde: \emph{como o tópico varia por X?}
  \item Custa mais: exige especificar a covariável.
\end{itemize}
\end{columns}
\vspace{0.2cm}
\begin{alertblock}{Regra prática}
\small Pergunta é \emph{quais tópicos existem}? Use LDA
(\texttt{ac\_lda()}). Pergunta já é \emph{como o tópico varia por
partido/período}? Comece direto no STM: comparar tópicos de um LDA plano
depois é mais frágil do que modelar a covariável desde o início.
\end{alertblock}
\vspace{0.1cm}
{\scriptsize Roberts, Stewart e Tingley. \texttt{stm}: An R Package for
Structural Topic Models. \textit{J. Statistical Software}, v. 91, n. 2, 2019.}
\end{frame}
...
\begin{frame}[fragile]{Descobrir tipos de documento: clustering}
\begin{columns}[T]
\column{0.44\textwidth}
\begin{lstlisting}
...
\end{lstlisting}
\footnotesize \textbf{Diferença para o LDA:} soft (mistura de tópicos)
vs. \emph{hard} (uma única etiqueta por documento).
\column{0.55\textwidth}
\centering
\includegraphics[width=0.92\linewidth]{figuras/fig_cluster.png}\\
\scriptsize \textit{Saída ilustrativa, gerada para fins didáticos.}
\end{columns}
\footnotesize \textbf{Quando usar:} tipologias latentes, amostra de
revisão estratificada, ou dendrograma no lugar de tópicos.
\texttt{hclust} (padrão), \texttt{kmeans} ou \texttt{pam}.
\end{frame}
...
\begin{frame}{$\kappa$ baixo com prevalência desbalanceada: o que fazer}
\footnotesize \textbf{O paradoxo (Feinstein e Cicchetti, 1990):} com
prevalência desbalanceada, dá para ter concordância bruta altíssima e
$\kappa$ perto de zero, ou negativo. O problema é a distribuição marginal,
não a codificação.
\vspace{0.15cm}
\begin{alertblock}{As quatro ações, em ordem}
\begin{enumerate}\footnotesize
  \item \textbf{Reporte em conjunto:} concordância bruta, $\kappa$/$\alpha$
        e AC1 de Gwet, que não sofre o mesmo colapso com categorias
        desbalanceadas.
  \item \textbf{Apresente a matriz de confusão:} mostra onde está o
        desacordo, o que nenhum número agregado mostra sozinho.
  \item \textbf{Sobreamostre as categorias raras} no teste de
        confiabilidade, para ter casos suficientes delas.
  \item \textbf{Nunca reporte só o número que favorece seu argumento.} Se
        $\kappa$ é ruim mas AC1 é bom, reporte os dois e explique.
\end{enumerate}
\end{alertblock}
\vspace{0.1cm}
\footnotesize \textit{Para a tarde:} o \texttt{acR} calcula todas essas
métricas simultaneamente, com intervalo de confiança por \emph{bootstrap}.
\end{frame}
