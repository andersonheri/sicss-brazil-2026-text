...
\textbf{13h55 a 14h30} & \textbf{Trabalho em grupos} &
\textbf{Codebook do seu projeto, pré-testado} \\
...
\small
Entregue o codebook a um colega que não conhece sua pesquisa, com cinco
documentos.\\[0.15cm]
Se as respostas dele divergirem das suas, o problema \textbf{nunca} é o colega.
É a definição.
\end{alertblock}
\end{columns}
\vspace{0.15cm}
\centering
\small É exatamente esse teste que vocês farão daqui a vinte minutos, com o
grupo vizinho.
\end{frame}
...
diagnóstico quando o pré-teste cruzado der desacordo.
\end{frame}
...
% --- SLIDE 7/10 ---------------------------------------------------------------
\begin{frame}[fragile]{Amostragem e métricas em quatro linhas}
\begin{lstlisting}
# Amostra por incerteza: prioriza onde o modelo hesitou
amostra <- ac_qual_sample(resultado, n = 150, strategy = "uncertainty")

# Planilha para os codificadores, SEM o rotulo do modelo (evita ancoragem)
...
\end{lstlisting}
\begin{lstlisting}[style=Rsaida]
<ac_qual_irr>  n = 150 | 2 codificadores (humano vs. LLM)
  Percent agreement      0.833   [0.771 , 0.888]
  Cohen's kappa          0.741   [0.652 , 0.821]
  Krippendorff's alpha   0.733   [0.641 , 0.815]
  Gwet's AC1             0.788   [0.712 , 0.856]
  F1 macro               0.793   [0.702 , 0.869]
\end{lstlisting}
\end{frame}
...
\begin{lstlisting}[style=Rsaida]
  ...
  punitivista       61     0.881     0.918    0.899    0.812
  preventivo        43     0.805     0.767    0.786    0.702
  garantista        17     0.632     0.471    0.540    0.489   <- fraco
  nao_aplicavel     29     0.933     0.966    0.949    0.914
\end{lstlisting}
\vspace{0.25cm}
\begin{alertblock}{O que este quadro autoriza, e o que ele proíbe}
\small
$\kappa = 0{,}74$ é publicável. Mas ``garantista'' tem revocação de 0,47: o
modelo perde metade dos casos.\\[0.12cm]
...
\begin{frame}{Atividade: 35 minutos, em grupos de três ou quatro}
\begin{block}{Objetivo}
Sair daqui com o esqueleto validável de uma medida de texto para \textbf{a
pesquisa de um de vocês}, já submetido a um teste de confiabilidade.
\end{block}
\vspace{0.15cm}
\begin{table}
\scriptsize
...
\midrule
5 min  & Cada um apresenta a frase do almoço; o grupo escolhe um projeto &
...
10 min & \textbf{Pré-teste cruzado} com o grupo vizinho &
5 documentos codificados com o codebook alheio \\
...
\end{table}
\vspace{0.05cm}
\footnotesize Materiais: \texttt{scripts/06\_atividade.R} e
\texttt{data/corpus\_atividade.csv}. Quem preferir papel, use papel: o produto é
o raciocínio, não o arquivo.
\end{frame}

\begin{frame}{Como conduzir o pré-teste cruzado}
\begin{columns}[T]
\column{0.49\textwidth}
\textbf{Regras}
\begin{enumerate}\small
  \item O grupo vizinho recebe \textbf{apenas} o codebook, sem explicação oral.
        Se precisar de explicação, o codebook está incompleto.
  \item Cada grupo codifica os mesmos 5 documentos, individualmente, antes de
        discutir.
  \item Só então comparem.
\end{enumerate}
\column{0.49\textwidth}
\textbf{O cálculo, sem software}
\vspace{0.1cm}

\small Concordância bruta $=$ acordos dividido por 5.
\vspace{0.15cm}
...
\small
Para cada discordância: os dois codificadores leram a mesma coisa e aplicaram
regras diferentes, ou leram coisas diferentes? O primeiro caso é problema de
\textbf{definição}. O segundo, de \textbf{unidade de registro}.
\end{alertblock}
\end{frame}

% --- APRESENTAÇÕES (14h30 às 14h50) ------------------------------------------
\begin{frame}{Apresentações: 3 minutos por grupo}
...
\begin{enumerate}\small
  ...
  \item A categoria com maior desacordo no pré-teste, e o diagnóstico:
  ...
  \item Qual método vocês usariam, entre dicionário, supervisionado e LLM, e por
        quê. \hfill (30 s)
\end{enumerate}
\vspace{0.25cm}
\begin{block}{Papel da plateia}
\small
Cada grupo que apresenta recebe \textbf{uma} pergunta do grupo que o auditou.
Pergunta, não sugestão. A melhor costuma ser: ``que documento faria essa
categoria falhar?''
\end{block}
\end{frame}
...
\centering
\large
Um grupo que apresenta um desacordo bem diagnosticado\\
aprendeu mais do que um grupo\\
com concordância perfeita em cinco documentos fáceis.
\vspace{0.5cm}

\small Se o seu codebook sobreviveu intacto ao pré-teste, desconfie: ou as
...
ac_qual_report(resultado, irr = irr, codebook = codebook,
               path = "outputs/relatorio_metodologico.html")
